...
export const FormCombobox: React.FC<FormComboboxProps> = ({ value, onChange, placeholder, disabled, searchLabel, fetchOptions, itemOptions }) => {
  ...
  return (
    <div className="w-full">
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger
          asChild
          className="h-11 w-full border px-3 text-base shadow-sm transition-colors rounded-md placeholder:text-muted-foreground disabled:cursor-not-allowed disabled:opacity-50 md:text-sm bg-background text-foreground border-input"
        >
          <Button variant="outline" role="combobox" aria-expanded={open} className={cn('w-full justify-between', disabled && 'opacity-50 cursor-not-allowed')} disabled={disabled}>
