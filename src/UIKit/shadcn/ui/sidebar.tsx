...
const SidebarInset = React.forwardRef<HTMLDivElement, React.ComponentProps<'main'>>(({ className, ...props }, ref) => {
  ...
  const sidebarWidth = open ? 'md:w-[calc(100%-var(--sidebar-width))]' : 'md:w-[calc(100%-var(--sidebar-width-icon))]'

  return (
    <main
      ref={ref}
      className={cn(
        'relative flex min-h-svh flex-1 flex-col bg-background overflow-hidden transition-[width] duration-300 ease-in-out',
...
const SidebarInput = React.forwardRef<React.ComponentRef<typeof Input>, InputProps>(({ className, ...props }, ref) => {
...
const SidebarMenuSub = React.forwardRef<HTMLUListElement, React.ComponentProps<'ul'>>(({ className, ...props }, ref) => (
  <ul
    ...
    className={cn('ml-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-l border-sidebar-border pl-2.5 pr-0.5 py-0.5', 'group-data-[collapsible=icon]:hidden', className)}
    {...props}
  />
))
