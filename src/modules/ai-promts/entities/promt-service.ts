...
export const promtService = {
  ...
  blind_tasting_note: {
    list: (): Promise<OpenAIFeatureConfig> => api.get(PROMT_ENDPOINTS.BLIND_TASTING.LIST).then(response => response.data),
    update: (params: OpenAIUpdateConfig): Promise<OpenAIFeatureConfig> => api.patch(PROMT_ENDPOINTS.BLIND_TASTING.UPDATE, params).then(response => response.data),
    reset: (): Promise<OpenAIFeatureConfig> => api.post(PROMT_ENDPOINTS.BLIND_TASTING.RESET).then(response => response.data),
  },
  scanner: {
