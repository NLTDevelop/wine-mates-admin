...
import { AnalysisReviewsResponse } from '../entities/types'
import { useAnalysisStore } from '../../entities/analysis-store'
import { analysisQueries } from '../../entities/analysis-queries'

export const mockReviewsResponse: AnalysisReviewsResponse = {
  count: 127,
  totalPages: 13,
  rows: [
    {
...
export const useReviews = () => {
  const { reviewFilters, setReviewFilters } = useAnalysisStore()

  const reviewsQuery: UseQueryResult<AnalysisReviewsResponse | undefined, Error> = useQuery(analysisQueries.reviews(reviewFilters))
