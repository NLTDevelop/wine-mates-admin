...
export const TasteCharacteristicsPaletteManager = () => {
  ...
  return (
    <Card>
      <CardContent className="space-y-2 sm:space-y-6 max-sm:p-0 sm:p-0">
        <div>
          <CreateTasteCharacteristicSection onCreateCharacteristic={characteristics.handleAddCharacteristic} isLoading={isLoading || isReordering} />
        </div>
                    ...
                    header={
                      <div className="flex justify-between items-center w-full pl-8 **min-w-0 flex-1** ">
                        <div className="flex md:items-center items-start gap-2 md:flex-row flex-col flex-1 min-w-0">
                          <span className={cn('font-medium truncate min-w-0 flex-1 text-start whitespace-nowrap', cardTextColorClass)}>
                            {nameUa} ({nameEn})
                          </span>
                        </div>
                        <PaletteItemActions
