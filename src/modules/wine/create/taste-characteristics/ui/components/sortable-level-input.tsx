...
interface SortableLevelInputProps {
  state: LevelItem
  onUpdate: (id: string, nameUa: string) => void
  onRemove: (id: string) => void
  isRequired: boolean
  onBlur?: (id: string, nameUa: string) => void
}
...
export const SortableLevelInput: React.FC<SortableLevelInputProps> = ({ state, onUpdate, onRemove, isRequired, onBlur }) => {
  ...
  const handleInputChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      if (state.id) onUpdate(state.id, e.target.value)
    },
    [state.id, onUpdate]
  )
  ...
  const handleInputBlur = useCallback(
    (e: React.FocusEvent<HTMLInputElement>) => {
      if (state.id) onBlur?.(state.id, e.target.value)
    },
    [state.id, onBlur]
  )

  const handleRemoveClick = React.useCallback(() => {
    if (state.id) onRemove(state.id)
  }, [state.id, onRemove])
  ...
  return (
    <div ref={setNodeRef} style={style} className="flex items-center gap-2 w-full">
      ...
      <div className="w-full">
        <Input value={state.nameUa} onBlur={handleInputBlur} onChange={handleInputChange} placeholder={t('taste_characteristics.entry_level')} className="flex-1 h-8" />
      </div>
