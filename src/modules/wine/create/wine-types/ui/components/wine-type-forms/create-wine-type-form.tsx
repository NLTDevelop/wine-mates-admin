...
interface CreateWineTypeFormProps {
  onSubmit: (wineType: CreateWineTypeParams) => void
  onCancel: () => void
  isLoading: boolean
}

export const CreateWineTypeForm = ({ onSubmit, onCancel, isLoading }: CreateWineTypeFormProps) => {
  const {
    formData,
    handleSubmit,
    handleChange,
    ...
    <Card className="border-l-4 border-l-green-500">
      <CardHeader className="pb-3">
        <CardTitle className="flex justify-between items-center">
          ...
          <Button variant="ghost" size="sm" onClick={onCancel} disabled={formLoading}>
            <X className="w-4 h-4" />
          </Button>
        </CardTitle>
      </CardHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input id="label" value={formData.label} onChange={e => handleChange('label', e.target.value)} placeholder="Красное, Белое, Розовое..." required disabled={formLoading} />
            </div>

            <div className="space-y-2">
              <Label htmlFor="labelEn">Английское название</Label>
              <Input id="labelEn" value={formData.labelEn} onChange={e => handleChange('labelEn', e.target.value)} placeholder="Red, White, Rosé..." disabled={formLoading} />
            </div>
          </div>
