...
export const useWineOptionsMock = () => {
  const fetchColors = useCallback(async (search?: string) => {
    return mockColors.filter(color => 
      !search || color.label.toLowerCase().includes(search.toLowerCase())
    )
  }, [])

  const fetchAromas = useCallback(async (search?: string) => {
    return mockAromas.filter(aroma => 
      !search || aroma.label.toLowerCase().includes(search.toLowerCase())
    )
  }, [])

  const fetchFlavorNotes = useCallback(async (search?: string) => {
    return mockFlavorNotes.filter(note => 
      !search || note.label.toLowerCase().includes(search.toLowerCase())
    )
  }, [])

  const fetchFlavorCharacteristics = useCallback(async (search?: string) => {
    return mockFlavorCharacteristics.filter(char => 
      !search || char.label.toLowerCase().includes(search.toLowerCase())
    )
  }, [])
