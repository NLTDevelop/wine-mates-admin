...
export const useWineFlavors = () => {
  ...
  const { data: flavors = [], isLoading } = useQuery(
    wineFlavorQueries.list()
  );

  const createMutation = useMutation(
    wineFlavorQueries.create()
  );

  const updateMutation = useMutation(
    wineFlavorQueries.update()
  );

  const deleteMutation = useMutation(
    wineFlavorQueries.delete()
  );
  ...
  const searchFlavors = (searchTerm: string) => {
    return flavors.filter(flavor =>
      flavor.label.toLowerCase().includes(searchTerm.toLowerCase()) ||
      flavor.items?.some(item =>
