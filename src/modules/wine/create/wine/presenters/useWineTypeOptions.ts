...
//   }
// }
// useWineTypeOptions.ts - исправленная версия
export const useWineTypeOptions = ({ cachedWineTypes, initialWineTypeId = null, onWineTypeChange }: UseWineTypeOptionsProps) => {
  const fetchOptions = useCallback(
    async (search?: string) => {
      const options = cachedWineTypes.map(wt => {
        ...
        return {
          value: wt.id.toString(),
          label: label,
          // Добавляем callback для выбора
          onSelect: () => onWineTypeChange(wt.id.toString()),
        }
      })
    ...
    [cachedWineTypes, onWineTypeChange] // ← добавляем onWineTypeChange в зависимости
  )
