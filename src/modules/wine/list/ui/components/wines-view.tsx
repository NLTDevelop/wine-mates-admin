...
export const WineView = () => {
  ...
  const filterParams = { sortBy, handleSort, handleColumnFilter, filters }
  ...
  const columns = useWineColumns({ onEdit: wine => navigate(`${getWineDetailPath(wine.id)}?edit=true`), onDelete: deleteWine, onConfirm: confirmModal.open, filterParams })
  ...
  const onChangePaginationWithReset = useCallback(
    (page: number) => {
      onChangePagination(page)
      table?.resetRowSelection()
    },
    [onChangePagination, table]
  )
  ...
  return (
    <ContentLayout title={t('list.wines_list')}>
      ...
      {totalCount && totalCount > DEFAULT_PAGINATION_LIMIT ? <NLTTablePagination limit={filters.limit} page={filters.page} totalRows={totalCount || 1} setPage={onChangePaginationWithReset} /> : null}
      <WarningModal
