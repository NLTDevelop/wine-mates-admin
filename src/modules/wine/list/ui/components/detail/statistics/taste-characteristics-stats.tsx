...
export default function TasteCharacteristicsStats({ tasteCharacteristics, height = 12 }: TasteStatsProps) {
  ...
  return totalUsers > 0 ? (
    <Card className="w-full space-y-3 mt-4">
      ...
      >
        {sortedCharacteristics.map((char, idx) => {
          ...
          return (
            <Card key={char.id} className="space-y-3 !w-full min-w-0 p-4 bg-accent">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full border border-gray-300 shadow-sm" style={{ backgroundColor: char.colorHex }} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className={`text-foreground ${idx === 0 && 'font-bold'} truncate`}>{char.name}</span>
                    <span className="text-gray-500 text-sm whitespace-nowrap">({t('user', { count: char.userCount })})</span>
                  </div>
                  <div className="flex items-center justify-between mt-1">
                    <span className="text-xs text-gray-500 font-mono">{char.colorHex}</span>
                      ...
                      <div key={level.id} className="space-y-1 p-2 bg-gray-50 rounded">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2 flex-1 min-w-0">
                            ...
                            />
                            <span className={`text-sm ${i === 0 && 'font-bold'} truncate flex-1 min-w-0`}>{level.name}</span>
                          </div>
